...
function showDropDown() {
  document
    .querySelector(".course-drop-list")
    .classList.toggle("course-drop-list-show");
}
...
class courseAndFees {
  ...
  handleChange(event) {
    ...
    if (document.getElementById(event.target.id).checked == true) {
      this.fees[this.courseDetails[event.target.id].course_id] =
        this.courseDetails[event.target.id].fees;
      this.pdfList[this.courseDetails[event.target.id].course_id] =
        this.courseDetails[event.target.id].pdf;
    } else {
    ...
    ).innerHTML = `${this.courseId.length} Courses Selected `;
  }
}
